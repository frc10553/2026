...
public class RobotContainer {
    ...
    private double SlowSpeed = TunerConstants.kSlowSpeedAt12Volts.in(MetersPerSecond); // kSlowSpeedAt 12Volts desired top speed 
            ...
            // Drivetrain will execute this command periodically
                drivetrain.applyRequest(() ->
                    drive.withVelocityX(-joystick.getLeftY() * SlowSpeed) // Drive forward with negative Y (forward)
                        ...
                        .withRotationalRate(-joystick.getRightX() * SlowAngularRate) // Drive counterclockwise with negative X (left)
                )   
            );
        } else {
            drivetrain.setDefaultCommand(
